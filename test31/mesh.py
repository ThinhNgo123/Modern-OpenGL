...
class Mesh:
    ...
    def setup_batch_mesh(self):
        ...
        positions = array(positions, dtype=np.float32)
        normals = array(normals, dtype=np.float32)
        tex_coords = array(tex_coords, dtype=np.float32)
        glBufferData(
            GL_ARRAY_BUFFER, 
            positions.nbytes + normals.nbytes + tex_coords.nbytes,
            None,
            GL_STATIC_DRAW
        )
        ...
        glBufferSubData(GL_ARRAY_BUFFER, positions.nbytes + normals.nbytes, tex_coords.nbytes, tex_coords)
        ...
        glEnableVertexAttribArray(0)
        glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * size, c_void_p(0))

        glEnableVertexAttribArray(1)
        glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, 3 * size, c_void_p(positions.nbytes))

        glEnableVertexAttribArray(2)
        glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, 2 * size, c_void_p(positions.nbytes + normals.nbytes))
